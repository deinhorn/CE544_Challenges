  ...
  if(timeDiff< updateRate || leader == 1){
   ...
   /*   Serial.print("cQueue: ");
    ...
    //non-leader is green (leader = 2);
      ...
      if (infection == 0){
        colorWipe(strip.Color(0, 255, 0), 1); //non-leader
        if(lQueue){
          leaderMessage = millis();
        } 
        if(iQueue && !cQueue){
          colorWipe(strip.Color(255, 0, 0), 1); //infect turn red
          infection = 1;
        ...
        //If receive clear infection message, change back to green
          //Serial.println(message);
          colorWipe(strip.Color(0, 255, 0), 1); //clear infection turn green
          infection = 0;
          three_sec = 1;
